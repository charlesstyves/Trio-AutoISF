...
extension Array where Element == CGRect {
    ...
    /// - Uses expanded obstacles (Minkowski sum) so we test point-in-rect for the center.
    /// - Tries same-row gaps, roofline slabs (above/below), and direct-over each obstacle.
    func placeLabelCenter(
        ...
        // A) SAME-ROW: merge overlapping x-blocks for obstacles covering cy, emit single candidate per gap
        do {
        ...
        // B) ROOFLINE: compute roof y near cx and emit a few nearby x samples
        do {
            switch verticalSide {
            case .above:
                roofline(for: .above)
        ...
        // C) DIRECT-OVER: probe points above/below obstacles near cx to break tight clusters
        do {
            switch verticalSide {
            case .above:
